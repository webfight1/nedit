...
<main class="site-main nailedit-cart-page   py-10">
    <div class="max-w-[1200px] mx-auto px-4">
        ...
        <div id="nailedit-cart-root" class="bg-white rounded-24 shadow-lg p-4 md:p-6 min-h-[160px] flex items-start justify-center flex-col">
            <p class="text-sm text-slate-500"><?php nailedit_t( 'loading_cart' ); ?></p>
        </div>
    </div>
</main>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderCart(data) {
            ...
            html += '<thead>' +
                '<tr class="bg-slate-100 text-slate-600 text-left text-[12px] uppercase tracking-wide">' +
                    '<th class="px-3 py-2 rounded-l-lg"><?php echo esc_js( nailedit_get_t( 'product' ) ); ?></th>' +
                    '<th class="px-3 py-2 text-center"><?php echo esc_js( nailedit_get_t( 'quantity' ) ); ?></th>' +
                    '<th class="px-3 py-2 text-right"><?php echo esc_js( nailedit_get_t( 'price' ) ); ?></th>' +
                    '<th class="px-3 py-2 text-right"><?php echo esc_js( nailedit_get_t( 'total' ) ); ?></th>' +
                    '<th class="px-3 py-2 text-right rounded-r-lg"><?php echo esc_js( nailedit_get_t( 'actions' ) ); ?></th>' +
                '</tr>' +
            '</thead>';
                ...
                html += '<tr data-item-id="' + itemId + '" class="hover:bg-slate-50 transition-colors">' +
                    ...
                    '<td class="px-3 py-3 align-middle text-right">' +
                        '<button class="nailedit-remove-item inline-flex items-center px-3 py-1.5 rounded-full border border-slate-300 text-[12px] text-slate-600 hover:border-primary hover:text-primary transition" data-item-id="' + itemId + '">' +
                            '<?php echo esc_js( nailedit_get_t( 'remove' ) ); ?>' +
                        '</button>' +
                    '</td>' +
                '</tr>';
            });
            ...
            if (grand) {
                html += '<div class="flex flex-col items-end">' +
                    '<div class="text-xs uppercase tracking-wide text-slate-500 mb-1"><?php echo esc_js( nailedit_get_t( 'order_summary' ) ); ?></div>' +
                    '<p class="text-[15px] md:text-[16px] text-slate-900"><span class="font-semibold mr-1"><?php echo esc_js( nailedit_get_t( 'total_amount' ) ); ?></span>' + grand + '</p>' +
                '</div>';
            }

            // Coupon form
            html += '<div class="mt-2 flex flex-col md:flex-row md:items-center md:justify-between gap-3">' +
                '<div class="text-sm text-slate-600"><?php echo esc_js( nailedit_get_t( 'have_coupon' ) ); ?></div>' +
                '<div class="flex flex-col items-stretch md:items-end gap-1">' +
                    '<div class="flex flex-wrap items-center gap-2">' +
                        '<input type="text" id="nailedit-cart-coupon" class="border border-slate-300 rounded-full px-3 py-1.5 text-sm focus:outline-none focus:border-primary min-w-[160px]" placeholder="<?php echo esc_js( nailedit_get_t( 'coupon_code' ) ); ?>" />' +
                        '<button type="button" id="nailedit-cart-apply-coupon" class="inline-flex items-center px-4 py-1.5 rounded-full gradient-secondary  text-slate-900 text-sm font-semibold hover:bg-fourth hover:text-white transition"><?php echo esc_js( nailedit_get_t( 'apply' ) ); ?></button>' +
                        '<button type="button" id="nailedit-cart-remove-coupon" class="inline-flex items-center px-3 py-1.5 rounded-full border border-slate-300 text-slate-600 text-xs font-medium hover:border-primary hover:text-primary transition"><?php echo esc_js( nailedit_get_t( 'remove_coupon' ) ); ?></button>' +
                    '</div>' +
            ...
            html += '<div class="mt-4 flex justify-end">' +
                '<a href="<?php echo esc_url( nailedit_get_url( 'checkout' ) ); ?>" class="inline-flex items-center px-5 py-2.5 rounded-full gradient-dark text-secondary  text-sm font-semibold  shadow-md hover:text-white transition">' +
                    '<?php echo esc_js( nailedit_get_t( 'go_to_checkout' ) ); ?>' +
                '</a>' +
            '</div>';
            ...
            removeButtons.forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    if (itemId && confirm('<?php echo esc_js( nailedit_get_t( 'confirm_remove_item' ) ); ?>')) {
            ...
            if (couponButton && couponInput && couponMsg) {
                couponButton.addEventListener('click', function() {
                    ...
                    if (!code) {
                        couponMsg.textContent = '<?php echo esc_js( nailedit_get_t( 'enter_coupon_code' ) ); ?>';
        ...
        function updateCartQuantity(cartItemId, quantity) {
            ...
            if (!cartItemId || isNaN(quantity) || quantity < 1) {
                alert('<?php echo esc_js( nailedit_get_t( 'quantity_min_one' ) ); ?>');
                return;
            }
            ...
            .then(function(data) {
                ...
                if (data && data.success) {
                    ...
                    const msg = (data && data.message) ? data.message : '<?php echo esc_js( nailedit_get_t( 'something_went_wrong' ) ); ?>';
                    alert(msg);
                }
            })
            .catch(function(err) {
                console.error('Update cart error:', err);
                alert('<?php echo esc_js( nailedit_get_t( 'error_updating_quantity' ) ); ?>');
            });
        }
        ...
        function applyCoupon(code, messageEl, buttonEl) {
            ...
            if (messageEl) {
                messageEl.textContent = '<?php echo esc_js( nailedit_get_t( 'applying_coupon' ) ); ?>';
            ...
            .then(function(data) {
                ...
                if (data && data.success) {
                    if (messageEl) {
                        const msg = (data.message) ? data.message : '<?php echo esc_js( nailedit_get_t( 'coupon_applied' ) ); ?>';
                        messageEl.textContent = msg;
                        messageEl.style.color = '#15803d';
                    }
                    loadCart();
                } else {
                    const msg = (data && data.message) ? data.message : '<?php echo esc_js( nailedit_get_t( 'could_not_apply_coupon' ) ); ?>';
                    if (messageEl) {
                        messageEl.textContent = msg;
            ...
            .catch(function(err) {
                ...
                if (messageEl) {
                    messageEl.textContent = '<?php echo esc_js( nailedit_get_t( 'error_applying_coupon' ) ); ?>';
                    messageEl.style.color = '#b91c1c';
                }
            })
            .finally(function() {
        ...
        function removeCoupon(messageEl, buttonEl) {
            ...
            if (messageEl) {
                messageEl.textContent = '<?php echo esc_js( nailedit_get_t( 'removing_coupon' ) ); ?>';
                messageEl.style.color = '#4b5563';
            }
            ...
            .then(function(data) {
                ...
                if (data && data.success) {
                    if (messageEl) {
                        const msg = (data.message) ? data.message : '<?php echo esc_js( nailedit_get_t( 'coupon_removed' ) ); ?>';
                        messageEl.textContent = msg;
                        messageEl.style.color = '#15803d';
                    }
                    loadCart();
                } else {
                    const msg = (data && data.message) ? data.message : '<?php echo esc_js( nailedit_get_t( 'could_not_remove_coupon' ) ); ?>';
                    if (messageEl) {
                        messageEl.textContent = msg;
                        messageEl.style.color = '#b91c1c';
                    }
            ...
            .catch(function(err) {
                ...
                if (messageEl) {
                    messageEl.textContent = '<?php echo esc_js( nailedit_get_t( 'error_removing_coupon' ) ); ?>';
                    messageEl.style.color = '#b91c1c';
                }
            })
            .finally(function() {
        ...
        function removeCartItem(cartItemId) {
            ...
            .then(function(data) {
                ...
                if (data && data.success) {
                    ...
                    const msg = (data && data.message) ? data.message : '<?php echo esc_js( nailedit_get_t( 'could_not_remove_item' ) ); ?>';
                    alert(msg);
                    // Re-enable buttons
            ...
            .catch(function(err) {
                ...
                alert('<?php echo esc_js( nailedit_get_t( 'error_removing_item' ) ); ?>');
                // Re-enable buttons
                allButtons.forEach(function(btn) { btn.disabled = false; });
            });
        }

        function loadCart(isRetry = false) {
            const formData = new FormData();
            ...
            .then(function(response) {
                ...
                if (response.status === 401 && !isRetry) {
                    console.log('Cart token invalid (401), clearing and retrying...');
                    localStorage.removeItem('bagisto_guest_cart_token');
                    localStorage.removeItem('bagisto_cart_cookie');
                    // Retry once without the invalid token
                    return loadCart(true);
                }
                return response.json();
            })
            .then(function(data) {
                // If we already retried, don't process undefined data
                ...
                if (!data || !data.success) {
                    ...
                    const msg = isGuestTokenError
                        ? '<?php echo esc_js( nailedit_get_t( 'cart_empty' ) ); ?>'
                        : (rawMessage || '<?php echo esc_js( nailedit_get_t( 'could_not_load_cart' ) ); ?>');
            ...
            .catch(function(err) {
                ...
                root.innerHTML = '<p><?php echo esc_js( nailedit_get_t( 'unexpected_error_loading_cart' ) ); ?></p>';
            });
        }
